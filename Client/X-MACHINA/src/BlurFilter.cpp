...
#pragma region Vertical Blur Pass
		mVertBlurShader->Set();
		cmdList->SetComputeRootDescriptorTable(scene->GetRootParamIndex(RootParam::Read), mOutput->GetGpuDescriptorHandle());
		cmdList->SetComputeRootDescriptorTable(scene->GetRootParamIndex(RootParam::Write), input->GetUavGpuDescriptorHandle());
