...
UINT LUTFilter::Execute(rsptr<Texture> input)
{
	...
	cmdList->SetComputeRootDescriptorTable(scene->GetRootParamIndex(RootParam::LUT0), scene->GetTexture("LUT_Sepia")->GetGpuDescriptorHandle());
	cmdList->SetComputeRootDescriptorTable(scene->GetRootParamIndex(RootParam::LUT1), scene->GetTexture("LUT_Sepia")->GetGpuDescriptorHandle());
